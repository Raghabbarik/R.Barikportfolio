...
const XConfig = {
  count: 30,
  colors: [0x8B5CF6, 0x3B82F6, 0x8B5CF6], // Purple, Blue, Purple
...
class Z extends InstancedMesh {
  ...
  #setupLights() {
    ...
    this.light = new PointLight(new Color(this.config.colors[0]), this.config.lightIntensity);
    this.add(this.light);
  }
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  return {
    three: threeInstance,
    spheres, // Changed to a getter in the original, but direct property is fine
...
const BallpitBackground: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
